...
INTERNAL=github.com/ectoclaw/ectoclaw/cmd/ectoclaw/internal
...
# Workspace
ECTOCLAW_HOME?=$(HOME)/.ectoclaw
WORKSPACE_DIR?=$(ECTOCLAW_HOME)/workspace
...
## build: Build the ectoclaw binary for current platform
build: generate
...
## build-all: Build ectoclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install ectoclaw to system and copy builtin skills
install: build
...
## uninstall: Remove ectoclaw from system
uninstall:
...
## uninstall-all: Remove ectoclaw and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(ECTOCLAW_HOME)
	@echo "Removed workspace: $(ECTOCLAW_HOME)"
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build ectoclaw-agent ectoclaw-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build ectoclaw-agent ectoclaw-gateway
...
## docker-run: Run ectoclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run ectoclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run ectoclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm ectoclaw-agent

## docker-run-agent-full: Run ectoclaw agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm ectoclaw-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi ectoclaw:latest ectoclaw:full 2>/dev/null || true

## help: Show this help message
help:
	@echo "ectoclaw Makefile"
	@echo ""
	@echo "Usage:"
	@echo "  make [target]"
	@echo ""
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.ectoclaw/workspace)"
