# EctoClaw Channel System: Complete Development Guide

> **Scope**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/ectoclaw/internal/gateway/`
...
├── interfaces.go        # Optional capability interfaces (TypingCapable, ReactionCapable, StateRecorder)
...
├── manager.go           # Unified orchestration: Worker queues, rate limiting, retries, Typing/Reaction, shared HTTP
...
└────────────┘                            └─────┬─────┘                        └────────────┘
                            ...
                            SubscribeOutbound() │  SubscribeOutboundMedia()
                                    ...
                                    │   └── runTTLJanitor()       Clean up expired Typing/Reaction entries
                                    └────────┬──────────┘
                                             │
                                   channel.Send()
                                             │
...
| **Capability Discovery** | Optional capabilities are declared via interfaces (`TypingCapable`, `ReactionCapable`, `WebhookHandler`, `HealthChecker`), discovered by Manager via runtime type assertions |
...
| **Centralized Orchestration** | Rate limiting, message splitting, retries, and Typing/Reaction management are all handled by Manager and BaseChannel; channels only need to implement Send |
...
import (
    "github.com/ectoclaw/ectoclaw/pkg/bus"
    "github.com/ectoclaw/ectoclaw/pkg/channels"     // Reference parent package
    "github.com/ectoclaw/ectoclaw/pkg/config"
    "github.com/ectoclaw/ectoclaw/pkg/identity"      // New
    "github.com/ectoclaw/ectoclaw/pkg/media"          // New (if media support needed)
)
```
...
```go
// cmd/ectoclaw/internal/gateway/helpers.go
import (
    _ "github.com/ectoclaw/ectoclaw/pkg/channels/telegram"   // Triggers init() registration
    _ "github.com/ectoclaw/ectoclaw/pkg/channels/discord"
    _ "github.com/ectoclaw/ectoclaw/pkg/channels/your_new_channel"  // New addition
)
```
...
| No Typing/Reaction management | Unified preSend handles Typing stop + Reaction undo; inbound-side BaseChannel.HandleMessage auto-orchestrates Typing/Reaction |
| No TTL cleanup | runTTLJanitor periodically cleans up expired Typing/Reaction entries |
...
### 3.4 Inbound-side Typing/Reaction Auto-orchestration

`BaseChannel.HandleMessage` automatically detects whether the channel implements `TypingCapable` and/or `ReactionCapable` **before** publishing the inbound message, and triggers the corresponding indicators:

```go
// Automatically executed inside BaseChannel.HandleMessage (no manual calls needed):
if c.owner != nil && c.stateRecorder != nil {
    // Typing — independent pipeline
    if tc, ok := c.owner.(TypingCapable); ok {
        if stop, err := tc.StartTyping(ctx, chatID); err == nil {
            c.stateRecorder.RecordTypingStop(c.name, chatID, stop)
        }
    }
    ...
    if rc, ok := c.owner.(ReactionCapable); ok && messageID != "" {
        if undo, err := rc.ReactToMessage(ctx, chatID, messageID); err == nil {
            c.stateRecorder.RecordReactionUndo(c.name, chatID, undo)
        }
    }
}
```
...
- Channels implementing `ReactionCapable` (Slack) do not need to manually call `AddReaction` + `RecordReactionUndo` in `handleMessage`
...
| `SetStateRecorder(r)` | StateRecorder injected by Manager |
...
StartAll:
  ...
  3. Start goroutines:
     - runWorker (per-channel outbound)
     - dispatchOutbound (route from bus to worker queues)
     - runTTLJanitor (every 10s clean up expired typing/reaction entries)
  4. Start shared HTTP server (if configured)

StopAll:
  1. Shut down shared HTTP server (5s timeout)
  2. Cancel dispatcher context
  3. Close worker queues → wait for drain to complete
  4. Stop each channel (channel.Stop)
```

#### Typing/Reaction Management

```go
// Manager implements StateRecorder interface
...
// Both are independent and do not interfere with each other. Channels don't need to call these manually.

// Outbound side: pre-send processing
func (m *Manager) preSend(ctx, name, msg) {
    key := name + ":" + msg.ChatID
...
5. **Typing/Reaction is handled by BaseChannel + Manager automatically**: A channel's `Send` method does not need to manage Typing stop or Reaction undo. `BaseChannel.HandleMessage` auto-triggers `TypingCapable` and `ReactionCapable` on the inbound side (via `owner` type assertions); Manager's `preSend` auto-stops Typing and undoes Reaction on the outbound side. Channels only need to implement the corresponding interfaces.
...
- `Manager.typingStops` / `Manager.reactionUndos`: Uses `sync.Map`
...
| `pkg/channels/interfaces.go` | TypingCapable, ReactionCapable, StateRecorder interfaces |
...
|-------------|----------------|-------------------|
| `pkg/channels/telegram/` | `"telegram"` | TypingCapable, MediaSender |
| `pkg/channels/discord/` | `"discord"` | TypingCapable, MediaSender |
...
// 3. Create Channel Manager (triggers initChannels → factory lookup → construct → inject MediaStore/StateRecorder/Owner)
...
2. **WhatsApp has two modes**: `"whatsapp"` (Bridge mode, communicates via external bridge URL) and `"whatsapp_native"` (native whatsmeow mode, connects directly to WhatsApp). Manager selects which to initialize based on `WhatsAppConfig.UseNative`.

3. **ReasoningChannelID**: Most channel configs include a `reasoning_channel_id` field to route LLM reasoning/thinking output to a designated channel (WhatsApp, Telegram, Discord, Slack, LINE). `BaseChannel` exposes this via the `WithReasoningChannelID` option and `ReasoningChannelID()` method.