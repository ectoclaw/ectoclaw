...
var (
	ircReCodeBlock = regexp.MustCompile("(?s)```[\\w]*\\n?([\\s\\S]*?)```")
	ircReInline    = regexp.MustCompile("`([^`]+)`")
	ircReBoldStar  = regexp.MustCompile(`\*\*(.+?)\*\*`)
	ircReBoldUnder = regexp.MustCompile(`__(.+?)__`)
	ircReItalic    = regexp.MustCompile(`_(.+?)_`)
	ircReStrike    = regexp.MustCompile(`~~(.+?)~~`)
	ircReLink      = regexp.MustCompile(`\[([^\]]+)\]\(([^)]+)\)`)
	ircReHeading   = regexp.MustCompile(`(?m)^#{1,6}\s+(.+)$`)
	ircReBlockquote = regexp.MustCompile(`(?m)^>\s?(.+)$`)
)
...
func stripMarkdown(text string) string {
	...
	// Strip code block fences, keep content.
	text = ircReCodeBlock.ReplaceAllString(text, "$1")

	// Strip inline code backticks, keep content.
	text = ircReInline.ReplaceAllString(text, "$1")

	// Strip headings, uppercase the content.
	text = ircReHeading.ReplaceAllStringFunc(text, func(m string) string {
		match := ircReHeading.FindStringSubmatch(m)
		if len(match) > 1 {
	...
	text = ircReLink.ReplaceAllString(text, "$1: $2")

	// Strip bold markers.
	text = ircReBoldStar.ReplaceAllString(text, "$1")
	text = ircReBoldUnder.ReplaceAllString(text, "$1")

	// Strip italic markers.
	text = ircReItalic.ReplaceAllString(text, "$1")

	// Strip strikethrough markers.
	text = ircReStrike.ReplaceAllString(text, "$1")

	// Strip blockquote prefix.
	text = ircReBlockquote.ReplaceAllString(text, "$1")
