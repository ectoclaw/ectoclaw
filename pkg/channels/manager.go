...
// preSend handles typing stop and reaction undo before sending a message.
func (m *Manager) preSend(ctx context.Context, name string, msg bus.OutboundMessage) {
	key := name + ":" + msg.ChatID
...
func (m *Manager) initChannel(name, displayName string) {
	...
	if err != nil {
		...
		// Inject StateRecorder if channel supports it
		if setter, ok := ch.(interface{ SetStateRecorder(r StateRecorder) }); ok {
			setter.SetStateRecorder(m)
		}
...
func (m *Manager) runWorker(ctx context.Context, name string, w *channelWorker) {
	...
	for {
		...
		case msg, ok := <-w.queue:
			...
			// Don't split messages that carry media — the channel handles presentation.
			maxLen := 0
			if len(msg.Parts) == 0 {
...
func (m *Manager) sendWithRetry(ctx context.Context, name string, w *channelWorker, msg bus.OutboundMessage) {
	...
	// Pre-send: stop typing and undo reaction
	m.preSend(ctx, name, msg)
