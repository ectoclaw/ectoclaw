...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("ectoclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@ectoclaw hello", want: true},
		{text: "hi @ectoclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bectoclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@ectoclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@ectoclaw:matrix.org hello", want: "hello"},
		{in: "@ectoclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@ectoclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@ectoclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@ectoclaw:matrix.org">EctoClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40ectoclaw%3Amatrix.org">EctoClaw</a> hello`,
			},
			want: true,
		},
	}
