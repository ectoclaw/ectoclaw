...
var (
	slackReHeading   = regexp.MustCompile(`(?m)^#{1,6}\s+(.+)$`)
	slackReLink      = regexp.MustCompile(`\[([^\]]+)\]\(([^)]+)\)`)
	slackReBoldStar  = regexp.MustCompile(`\*\*(.+?)\*\*`)
	slackReBoldUnder = regexp.MustCompile(`__(.+?)__`)
	slackReStrike    = regexp.MustCompile(`~~(.+?)~~`)
	slackReCodeBlock = regexp.MustCompile("(?s)```[\\w]*\\n?([\\s\\S]*?)```")
	slackReInline    = regexp.MustCompile("`([^`]+)`")
)
...
func markdownToSlackMrkdwn(text string) string {
	...
	// Extract code blocks to protect them from substitution.
	var codeBlockContents []string
	i := 0
	text = slackReCodeBlock.ReplaceAllStringFunc(text, func(m string) string {
		match := slackReCodeBlock.FindStringSubmatch(m)
		if len(match) > 1 {
			codeBlockContents = append(codeBlockContents, match[1])
		} else {
			codeBlockContents = append(codeBlockContents, "")
		}
	...
	// Extract inline code to protect it from substitution.
	var inlineContents []string
	j := 0
	text = slackReInline.ReplaceAllStringFunc(text, func(m string) string {
		match := slackReInline.FindStringSubmatch(m)
		if len(match) > 1 {
			inlineContents = append(inlineContents, match[1])
		} else {
			inlineContents = append(inlineContents, "")
		}
		placeholder := fmt.Sprintf("\x00IC%d\x00", j)
		j++
		return placeholder
	})

	// Convert headings to bold.
	text = slackReHeading.ReplaceAllString(text, "*$1*")

	// Convert links to Slack format.
	text = slackReLink.ReplaceAllString(text, "<$2|$1>")

	// Convert bold.
	text = slackReBoldStar.ReplaceAllString(text, "*$1*")
	text = slackReBoldUnder.ReplaceAllString(text, "*$1*")

	// Convert strikethrough.
	text = slackReStrike.ReplaceAllString(text, "~$1~")

	// Restore inline code.
	for k, code := range inlineContents {
		text = strings.ReplaceAll(text, fmt.Sprintf("\x00IC%d\x00", k), "`"+code+"`")
	}

	// Restore code blocks.
	for k, code := range codeBlockContents {
		text = strings.ReplaceAll(text, fmt.Sprintf("\x00CB%d\x00", k), "```"+code+"```")
	}
