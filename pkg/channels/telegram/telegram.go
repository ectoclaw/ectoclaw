...
func (c *TelegramChannel) Send(ctx context.Context, msg bus.OutboundMessage) error {
	...
	// Send text content — skip if empty and there are Parts (media-only message).
	if msg.Content != "" {
		// The Manager already splits messages to ≤4000 chars (WithMaxMessageLength),
		// so msg.Content is guaranteed to be within that limit. We still need to
		// check if HTML expansion pushes it beyond Telegram's 4096-char API limit.
		// Skip text send when it will be embedded as caption on the first Part below.
		embedAsCaption := len(msg.Parts) > 0 && len([]rune(msg.Content)) <= 1024
		if !embedAsCaption {
	...
	// Send media Parts — embed text as caption on first Part when it fits.
	if len(msg.Parts) > 0 {
		...
		for i, part := range msg.Parts {
			...
			rawCaption := part.Caption
			if i == 0 && rawCaption == "" && len([]rune(msg.Content)) <= 1024 {
				rawCaption = msg.Content
			}
			caption := markdownToTelegramHTML(rawCaption)
